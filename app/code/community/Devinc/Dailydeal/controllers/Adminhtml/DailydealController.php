		...
		if ($data = $this->getRequest()->getPost()) {
			...
			try {				
				...
				if ($in_stock && $product_status==1) {
					...
					if ($stockItem->getIsInStock()) {
						Mage::getSingleton('adminhtml/session')->addError(Mage::helper('dailydeal')->__('Deal was saved & disabled because the deal\'s qty is 0.'));		
					} else {
					...
					Mage::getSingleton('adminhtml/session')->addError(Mage::helper('dailydeal')->__('Deal was saved & disabled because no product was assigned.'));					
				}
